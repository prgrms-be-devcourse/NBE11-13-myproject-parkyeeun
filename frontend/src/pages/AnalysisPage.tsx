...
function AnalysisPage({
  ...
  const loadLatestAnalysis = async (date: string) => {
    setLoading(true);
    setErrorMessage("");
  ...
  useEffect(() => {
    if (
      !analysisJob ||
      !["PENDING", "RUNNING"].includes(analysisJob.status)
    ) {
      return;
    }
    ...
    <Layout>
      ...
      <section className="mt-6 rounded-xl border border-slate-200 p-5">
        ...
        <p className="mt-2 text-xs text-slate-500">
          학습 날짜는 한국 시간 오전 6시부터 다음 날 오전 6시
          이전까지의 커밋을 기준으로 분석합니다.
        </p>
      </section>
          ...
          {analysisJob.status === "COMPLETED" &&
            analysisJob.result && (
              <section className="mt-6">
                ...
                {analysisJob.result.commits.length === 0 ? (
                  ...
                  <ul className="mt-4 space-y-4">
                    {analysisJob.result.commits.map((commit) => (
                      <li
                        key={commit.sha}
                      ...
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            {commit.categories.map((category) => (
                              <span
                                key={category}
                        ...
                        <div className="mt-5">
                            {commit.files.length === 0 ? (
                                ...
                                <FileGroupList files={commit.files} />
                            )}
                        </div>
                      </li>
                    ))}
