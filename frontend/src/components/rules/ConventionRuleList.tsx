...
type ConventionRuleListProps = {
  rules: ConventionRule[];
  processingRuleId: number | null;
  onEdit: (rule: ConventionRule) => void;
  onToggleEnabled: (rule: ConventionRule) => Promise<void>;
  onDelete: (rule: ConventionRule) => Promise<void>;
};

function ConventionRuleList({
  rules,
  processingRuleId,
  onEdit,
  onToggleEnabled,
  onDelete,
}: ConventionRuleListProps) {
  if (rules.length === 0) {
    return (
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-sm text-slate-500">
                  <span>type: {rule.commitType}</span>
...
export default ConventionRuleList;